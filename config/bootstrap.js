					  ...
					  fs.readFile(file, 'utf8', function (err, file_data) {
						  ...
						  // and do whatever other string processing you want
						   ...
						   if (initial_result && initial_result.length>0){
								...
								var mjobid= initial_result[2];
								var commit_evaluate= initial_result[3];
								Game.findOne({
									name:gname
											...
											if (mygame.initial_solution.length==0 && (commit_evaluate=='c' || commit_evaluate=='C') ){
												Network.findOne({ where: { name: mygame.network_name }, select: ['junctions'] }).exec(function (nerr, mynet){
													var moverall_cost=parseFloat(initial_result[4]);
													var mleakage=parseFloat(initial_result[5]).toFixed(2);
													var mwater_age=parseFloat(initial_result[6]);
													var mpressure_diff=parseFloat(initial_result[7]);
												...
												if(commit_evaluate=='c' || commit_evaluate=='C'){
													Network.findOne({ where: { name: mygame.network_name }, select: ['junctions'] }).exec(function (nerr, mynet){
														var myteam= mygame.players_teams[myuser.name]['team'];
						  ...
						  // // here replace sessions by game name. game name needs to be extracted from file.
						  // // it might make sense to have a lighter version of game user history with just end scores and index reference to file line.
						  // var new_session={'sessions':[]}
						  // for(var line = 0; line < lines.length; line++){  
						    // new_session['sessions'].push(lines[line].split(' '))
						  // }	  
						  // //console.log(new_session['sessions'][new_session['sessions'].length-2]);
						  ...
						  // User.update({name:myuser.name},{history:new_session}).exec(function afterwards(err, updated){
						    ...
						    // io.socket.put('/update', {name: myuser.name ,history: new_session['sessions'][new_session['sessions'].length-2]},function gotResponse(body, response) {
							// console.log('Server sending request ot server ');
						    // })
